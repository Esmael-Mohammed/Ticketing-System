...
export default class TicketList extends Component {
  ...
  render() {
    ...
    return (
        ...
        {tickets &&tickets?.length === 0 ? <p>No tickets found.</p> : tickets?.map((ticket) => (
          <div key={ticket?._id} className="p-4 border rounded mb-2">
            ...
            {user?.role === "admin" && (
              <div className="mt-2">
                <button className="bg-green-500 text-white px-3 py-1 mr-2" onClick={() => this.handelChangeStatus(ticket._id, "in_progress")}>In Progress</button>
                <button className="bg-red-500 text-white px-3 py-1" onClick={() => this.handelChangeStatus(ticket._id, "closed")}>Close</button>
              </div>
            )}
          </div>
        ))}
